...
Logging of individual requests and responses is disabled by default to protect your data privacy.
...
There are also options to mask sensitive information and exclude certain requests from logging.

Apitally applies default masking and exclusion rules to common sensitive information like `Authorization` headers and common health check paths.
...
app.add_middleware(
    ...
    request_logging_config=RequestLoggingConfig(
        ...
        # Mask some custom headers with sensitive information (applied to both request and response headers)
        # Common sensitive headers like `Authorization` are masked by default
        mask_headers=[r"^X-Some-Secret$"],
...
Check out the [reference page](/reference/python) for more details about the request logging configuration options.
