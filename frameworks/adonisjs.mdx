...
Then configure Apitally by running the following Ace command:
...
After running the command, update the `.env` file with the correct `APITALLY_CLIENT_ID` and `APITALLY_ENV` values.
You'll find the client ID on the _Setup instructions_ page for your app in the Apitally dashboard, which is displayed immediately after creating the app.
