...
![Create app](https://assets.apitally.io/docs/create-app-litestar.png)
...
Alternatively, you can set the `consumer_identifier` attribute on the [`request.state`](https://docs.litestar.dev/latest/reference/datastructures.html#litestar.datastructures.State) object, for example in a `before_request` [life cycle hook](https://docs.litestar.dev/latest/usage/lifecycle-hooks.html#before-request) or directly in your route handler functions.
...
async def before_request_handler(request: Request) -> None:
    request.state.consumer_identifier = "user-123"
