...
If your application already registers its own tracer provider, add `ApitallySpanProcessor` to its span processors. For example, with `NodeSDK`:
...
Your existing exporters continue to receive spans, while Apitally captures spans created during requests.
