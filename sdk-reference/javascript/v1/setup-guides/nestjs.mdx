...
sidebarTitle: "NestJS"
description: "Set up the Apitally SDK for your NestJS application."
---
...
This page guides you through the setup of the Apitally SDK for your [NestJS](https://nestjs.com) application. If you don't have an Apitally account yet, [sign up](https://app.apitally.io/?signup) before getting started.
...
Call `useApitally` with your NestJS application, the write token, and [other configuration options](/sdk-reference/javascript/v1/configuration) immediately after `NestFactory.create`, before calling `app.init()` or `app.listen()`.

```typescript main.ts {8-11}
...
Call `setConsumer` during request handling to associate the current request with a consumer identifier. You can call it wherever the consumer is known, such as in a controller method, an interceptor, or authentication code.

You can also provide a display name and group for each consumer.

<CodeGroup>
```typescript Global interceptor {7-11}
...
```typescript Controller method {13-17}
...
export class ItemsController {
  ...
  listItems(
    @Req() request: {
      user?: { username: string; name?: string; group?: string };
    },
  ): string[] {
    if (request.user) {
      setConsumer({
