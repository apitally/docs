---
title: Configuration
description: How to configure the Apitally SDK for Go.
mode: wide
---
...
The `MaskRequestBodyCallback`, `MaskResponseBodyCallback` and `ExcludeCallback` fields accept a callback function that takes a `Request` struct as the first argument. This struct has the following fields:
...
The `MaskResponseBodyCallback` and `ExcludeCallback` callback functions also take a `Response` struct as the second argument. This struct has the following fields:
