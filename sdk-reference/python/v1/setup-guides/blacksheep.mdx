...
description: "Set up the Apitally SDK for your BlackSheep application."
---
...
This page guides you through the setup of the Apitally SDK for your [BlackSheep](https://www.neoteroi.dev/blacksheep/) application. If you don't have an Apitally account yet, [sign up](https://app.apitally.io/?signup) before getting started.

<SetupGuideStartSnippet framework="BlackSheep" />

## Requirements

Requires Python 3.10+ and BlackSheep 2.6.1+.

## Create app

To get started, create a new app in the [Apitally dashboard](https://app.apitally.io/apps) and select BlackSheep as your framework.
...
Install the Apitally SDK with the `blacksheep` extra as a dependency in your project.

<CodeGroup>
```shell pip
pip install --pre "apitally[blacksheep]"
```

```shell uv
uv add --prerelease allow "apitally[blacksheep]"
```

```shell poetry
poetry add --allow-prereleases "apitally[blacksheep]"
```
</CodeGroup>

## Initialize Apitally

Call `apitally.init` with your BlackSheep application, the write token, and [other configuration options](/sdk-reference/python/v1/configuration) immediately after creating the app.
...
```python Endpoint function {11-15}
import apitally
...
async def list_items(request: Request) -> Response:
    if request.identity and request.identity.is_authenticated():
        apitally.set_consumer(
            request.identity.sub,
