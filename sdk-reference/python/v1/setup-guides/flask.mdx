...
sidebarTitle: "Flask"
description: "Set up the Apitally SDK for your Flask application."
---
...
This page guides you through the setup of the Apitally SDK for your [Flask](https://flask.palletsprojects.com) application. If you don't have an Apitally account yet, [sign up](https://app.apitally.io/?signup) before getting started.

<SetupGuideStartSnippet framework="Flask" />

## Requirements

Requires Python 3.10+ and Flask 2.0+.
...
Install the Apitally SDK with the `flask` extra as a dependency in your project.

<CodeGroup>
```shell pip
pip install --pre "apitally[flask]"
```

```shell uv
uv add --prerelease allow "apitally[flask]"
```

```shell poetry
poetry add --allow-prereleases "apitally[flask]"
```
</CodeGroup>

## Initialize Apitally

Call `apitally.init` with your Flask application, the write token, and [other configuration options](/sdk-reference/python/v1/configuration) immediately after creating the app.
...
Call `apitally.set_consumer` during request handling to associate the current request with a consumer identifier. You can call it wherever the consumer is known, such as in a view, a request callback, existing middleware, or authentication code.

You can also provide a display name and group for each consumer.

<CodeGroup>
```python Before request hook {12-16}
import apitally
...
@app.before_request
def identify_consumer() -> None:
    user = g.get("user")
    if user:
        apitally.set_consumer(
            user["email"],
            name=user["name"],  # optional
            group=user["role"],  # optional
        )
```

```python View function {12-16}
import apitally
from flask import Flask, g
