...
Call `apitally.set_consumer` from a dependency or in your endpoint functions to associate the current request with a consumer identifier.
...
The SDK provides helper functions in `apitally.otel` to instrument popular libraries. Each requires a separate OpenTelemetry instrumentation package. For example, to trace database queries made with SQLAlchemy, install:
...
More information about tracing and the provided helpers is available [here](/sdk-reference/python/v1/tracing).
