...
Call `apitally.init` with the write token and [other configuration options](/sdk-reference/python/v1/configuration) at the end of your Django settings file (`settings.py`), after `MIDDLEWARE` is defined. The SDK adds its middleware automatically.
...
def list_items(request: Request) -> Response:
    if request.user.is_authenticated:
        apitally.set_consumer(
            ...
            group=request.user.groups.first().name,  # optional
        )
    ...
    def __call__(self, request: HttpRequest) -> HttpResponse:
        ...
        if user is not None and user.is_authenticated:
            apitally.set_consumer(
                ...
                group=user.groups.first().name,  # optional
            )
        return response
```
...
If you use the middleware example, save it in `your_project/api/middleware.py` and append it to `MIDDLEWARE` before calling `apitally.init`.
