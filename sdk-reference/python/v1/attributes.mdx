...
You can use this metadata to make decisions in [sampling](/sdk-reference/python/v1/sampling#custom-sampling) and [masking](/sdk-reference/python/v1/masking#custom-body-masking-callbacks) callbacks, which receive the `span` object as an argument. Access attributes through `span.attributes`. Individual attributes might be missing, so use `.get()`:
