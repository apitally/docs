...
import GunicornUWSGIWarning from '/snippets/gunicorn-uwsgi-warning.mdx';
import RequestLoggingConfigSnippet from '/snippets/request-logging-config.mdx';
import AddMiddlewareCheck from '/snippets/add-middleware-check.mdx';
...
import ConfigureRequestLoggingCheck from '/snippets/configure-request-logging-check.mdx';
...
<div style={{ marginTop: "-0.5rem" }}>
  <GunicornUWSGIWarning framework="Django" />
</div>
...
To enable tracing, set `capture_traces` to `True` and instrument the libraries you want to trace with OpenTelemetry. See the [tracing guide](/sdk-reference/python/v0/tracing) for further instructions.
