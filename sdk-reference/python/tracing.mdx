...
The SDK provides helper functions to simplify instrumenting popular libraries and creating custom spans, and integrates seamlessly with existing OpenTelemetry setups.
...
Install the required packages for the libraries you want to instrument:
...
Call the instrumentation functions once at application startup:
...
Some functions accept an optional client, connection or engine argument to instrument a specific instance instead of all instances globally:
...
For custom operations that aren't covered by library instrumentation, you can create spans manually using the helpers provided by the SDK.
...
The span name is derived from the function name, and the span attributes include the file path and function location.

### `span()` context manager

Use the `span()` context manager to trace specific code blocks within a function:
...
If you have an existing OpenTelemetry setup, Apitally's span collector integrates seamlessly. The SDK registers itself as a span processor with the existing global `TracerProvider`, so any spans created using the standard OpenTelemetry API or any other instrumentation library will be captured automatically.
