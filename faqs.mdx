...
Only metadata about API requests and responses is recorded and aggregated before being sent to Apitally.
This includes the HTTP method used, the requested path, the response status code and timing.
The request and response bodies are never sent to Apitally, so you don't have to worry about exposing sensitive data.
