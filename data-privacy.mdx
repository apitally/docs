...
- Collect only the data that's required
- Require explicit user opt-in for features that could capture sensitive data
...
<Tip>
  ...
  Check out the [SDK reference](/sdk-reference/overview) for your language to see the equivalent parameter names.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title={<>Consumer <span style={{ position: "relative", top: -2, marginLeft: "0.25rem" }}><Icon icon="shield" /></span></>}>
    ...
    See the [setup guide](/sdk-reference/setup-guides/overview) for your framework for how to associate requests with consumers.
