...
<Tip>
  ...
  Check out the [reference](/reference/python) for your language to see the equivalent parameter names.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title={<>Consumer <span style={{ position: "relative", top: -2, marginLeft: "0.25rem" }}><Icon icon="shield" /></span></>}>
    ...
    See the setup guide for your framework for how to associate requests with consumers.
...
You can add additional patterns via the `mask_body_fields` option.

## Data filtering

The Apitally SDK automatically excludes requests to common health check endpoints from the logs using the patterns below. They are applied to the request path.
