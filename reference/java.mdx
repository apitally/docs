...
| <nobr>`query-param-mask-patterns`</nobr> | List of regular expressions for matching query parameter names that should be masked. These are in addition to the default masking patterns. | `List<String>` | `[]` |
| <nobr>`header-mask-patterns`</nobr> | List of regular expressions for matching header names that should be masked. These are in addition to the default masking patterns. | `List<String>` | `[]` |
| <nobr>`body-field-mask-patterns`</nobr> | List of regular expressions for matching request/response body fields that should be masked. These are in addition to the default masking patterns. | `List<String>` | `[]` |
